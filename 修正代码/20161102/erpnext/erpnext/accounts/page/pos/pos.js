...
erpnext.pos.PointOfSale = erpnext.taxes_and_totals.extend({
	...
	get_data_from_server: function(callback){
		...
		frappe.call({
			...
			callback: function(r){
				...
				// tangxuejun 20161031 add stt
				window.gst_number = r.message.gst_number;
				window.warehouse = r.message.warehouse;
				// tangxuejun 20161031 add end
	...
	load_data: function(load_doc){
		...
		// tangxuejun 20161031 add stt
		this.frm.doc.warehouse = window.warehouse;
		this.frm.doc.gst_number = window.gst_number;
		this.frm.doc.terms = window.gst_number;
		// tangxuejun 20161031 add end
		...
		this.party_field.$input.autocomplete({
			...
			source: function (request, response) {
				...
				response($.map(me.customer_data, function(data){
					// 20161102 tangxuejun modify stt
					//return {label: data.name, value: data.name,
					//	customer_group: data.customer_group, territory: data.territory}
					return {label: (data.name + " : " + data.customer_group), value: data.name,
						customer_group: data.customer_group, territory: data.territory}
					// 20161102 tangxuejun modify end
				}))
			},
			change: function(event, ui){
				if(ui.item){
					...
					me.frm.doc.customer = ui.item.value;
					// 20161102 tangxuejun modify end
	...
	add_new_item_to_grid: function() {
		...
		// 20161010 tangxuejun add stt
		this.child.price_list_by_code = this.items[0].price_list_by_code;
		// 20161010 tangxuejun add end
	...
	show_items_in_item_cart: function() {
		...
		$.each(this.frm.doc.items|| [], function(i, d) {
			$(frappe.render_template("pos_bill_item", {
				...
				item_name: (d.item_name===d.item_code || !d.item_name) ? "" : ("<br>" + d.item_name + "<br>" + d.price_list_by_code),

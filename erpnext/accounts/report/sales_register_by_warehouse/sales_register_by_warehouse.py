...
def execute(filters=None):
	...
	mode_of_payments_amount = get_mode_of_payments_amount(filters)
	...
	for inv in invoice_list:
		...
		if filters.get("mode_of_payment"):
			...
			inv.debit_to, ", ".join(mode_of_payments.get(inv.name, [])), float(mode_of_payments_amount.get(inv.name, [])[0]), inv.remarks, 
			", ".join(sales_order), ", ".join(delivery_note), company_currency]
		else:
			row = [inv.name, inv.posting_date, inv.customer, inv.customer_name,
			customer_map.get(inv.customer, {}).get("customer_group"), 
			customer_map.get(inv.customer, {}).get("territory"),
			inv.debit_to, ", ".join(mode_of_payments.get(inv.name, [])), "0.00", inv.remarks, 
			", ".join(sales_order), ", ".join(delivery_note), company_currency]
...
def get_columns(invoice_list):
	...
	columns = [
		...
		_("Payment Amount") +":Currency/currency:120", _("Remarks") + "::150", 
...
def get_mode_of_payments_amount(filters):
	mode_of_payments_amount = {}
	if not filters.get("mode_of_payment"):
		return mode_of_payments_amount
	
	inv_mop = frappe.db.sql("""select parent, amount
		from `tabSales Invoice Payment` 
		where docstatus = '1' and ifnull(mode_of_payment, '') = %s """, filters.get("mode_of_payment"), as_dict=1)
	print("filters.get(mode_of_payment):" + filters.get("mode_of_payment"))
	for d in inv_mop:
		print("d.amount:" + str(d.amount))
		mode_of_payments_amount.setdefault(d.parent, []).append(d.amount)
